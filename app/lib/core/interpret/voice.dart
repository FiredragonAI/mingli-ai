...
String loveHeadline(BaziChart c, String pattern, int overall) {
  final seed = personSeed(c) + overall;
  final pool = switch (pattern) {
    '晚成型' => ['你的缘分不是没来,是在路上挑日子', '晚一点遇到的,往往是对的那个', '命里配偶星不显,不等于没戏——等风来'],
    '选择丰富型' => ['你不缺人追,缺一条自己的标准', '桃花多是好事,难的是别看花了眼', '感情经历比别人丰富,这是阅历不是负担'],
    '主动追求型' => ['你习惯主动,记得也给对方追你的机会', '付出是优点,但别把它当筹码', '你追人很行,被追的时候别慌'],
    '被动吸引型' => ['异性缘从来不是你的问题,"选"才是', '找让你省力的人,不是让你心跳的人', '你容易在关系里累——因为你总选难的'],
    '需要空间型' => ['你需要的不是管你的人,是欣赏你的人', '你的光会晃到对方,找个不怕光的', '别为了迁就而收起才华,那不是爱'],
    '朋友变恋人型' => ['你的爱情多半从"哥们儿"开始', '先做朋友再谈,是你最大的优势', '熟人圈里有你的缘分,也有你的对手'],
    '慢热依赖型' => ['你慢热,但一旦认定就很稳', '需要被照顾不丢人,选个有耐心的', '你的爱是文火,别找急火的人'],
    _ => ['你是能"过日子"的配置', '婚姻质量在你手里,不在运气里', '稳定是你的底色,别把它活成无聊'],
  };
  return pick(pool, seed);
}

String loveQuote(BaziChart c, LoveLike f) {
  final seed = personSeed(c) + f.overall * 3;
  final pool = <String>[
    '夫妻宫是你的"位置",配偶星是坐上来的人——${f.starState == '无' ? '位置空着,人在路上' : f.starState == '杂' ? '来的人不止一个' : '位置和人都在'}。',
    '八字看婚姻,看的是"你会被什么样的人吸引",不是"你会嫁给谁"。',
    if (f.windowYears.isNotEmpty) '${f.windowYears.first} 年前后留意身边的人,那年你的缘分线亮着。',
    '感情模式是"默认设置",知道了就能改。',
    '命盘给的是天气预报,带不带伞、去不去,还是你说了算。',
  ];
  return pick(pool, seed);
}

/// 单人婚缘"别做"。
String loveDont(BaziChart c, LoveLike f) {
  final pool = <String>[
    if (!f.palaceStable) '别在争吵时提"分开",你的夫妻宫带冲,说了容易成真。',
    if (f.starState == '杂') '别同时暧昧两个人,你的命盘扛不住这种复杂。',
    if (f.starState == '无') '别为了"到年纪了"随便定下来,你的缘分在后面。',
    if (f.stars.contains('孤辰') || f.stars.contains('寡宿')) '别用"我一个人也挺好"当挡箭牌。',
    '别拿命盘去要求对方,它是照自己的镜子。',
    '别在感情里做"猜"这件事,直接问。',
  ];
  return pick(pool, personSeed(c) + f.overall * 7);
}

/// 让 voice.dart 不依赖 love_forecast.dart 的具体类型。
abstract class LoveLike {
  int get overall;
  String get starState;
  bool get palaceStable;
  List<String> get stars;
  List<int> get windowYears;
}
